(ns food.client
  (:require [cljsjs.semantic-ui]
            ...
            [food.types :as t]
            [food.render :refer [render destruct]]
...
(defn view [server-state client-state]
  [
   (topbar/create :topbar client-state :sidebar)
    ["div" {:merge (d3 (attr "class" "ui bottom attached segment pushable"))}
     (sidebar/create :sidebar client-state server-state)
     ["div" {:merge (d3 (attr "class" "pusher")
      ...
      ["div" {:merge (d3 (attr "class" "ui basic segment"))}
       (for [{:keys [image title]} (get-in @client-state [:search-result :list])]
         ["img" {
                 :id title
                 :merge (d3 (attr "src" image))
                 }
          ]
         )
       ]
