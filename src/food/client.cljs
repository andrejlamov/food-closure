(ns food.client
  (:require [cljsjs.semantic-ui]
            ...
            [cljs.core.async :as async
             :refer [sub pub <! >! chan close! sliding-buffer put! alts!]]
...
(def top-items (atom ["firefox"
                      "chrome"
                      "opera"]))
(def list-items (atom ["chrome"]))
...
(def input-chan (chan))
(def our-pub (pub input-chan :name))
   ...
   [:div.ui.top.attached.menu
    (for [n @top-items]
      [:div.ui.icon.item
        ...
        :enter (d3 (each (fn []
                           (this-as this
                             (let [output-chan (chan)]
                               (sub our-pub (keyword n) output-chan)
                               (go-loop []
       ...
       [:i.huge.icon {:join (d3 (classed n true)
                                (classed (str "topbar-" n) true))
                      ;; :enter (d3 (each (fn []
                      ;;                    (when (not initial-render)
                      ;;                      (this-as this
   ...
   [:div.ui.bottom.attached.segment
    (for [n @list-items]
      [:div.ui.vertical.segment
       {:id n
        :click (fn [d]
                 (put! input-chan {:name (keyword n) :text "hi"})
                 (println n))
